...
type LinhaVinculo = { id: string; usuario_id: string; rede_id: string; operacao_id: string | null; area_operacional_id: string | null; empresa_id: string | null; unidade_id: string | null; escopo_tipo: CodigoEscopoAcesso; perfil_id: string; ativo: boolean; criado_em: string };
...
type ConsultarContextoRelacional = (usuarioId: string) => Promise<ResultadoConsultaContextoRelacional>;
...
async function consultarContextoRelacional(usuarioId: string): Promise<ResultadoConsultaContextoRelacional> {
...
function mapearContextoRelacional(linha: LinhaContextoRelacional, usuarioId: string): ContextoAcesso | null {
  if (!linha.ativo || linha.usuario_id !== usuarioId) return null;
...
export async function obterVinculoDoUsuario(usuarioId: string): Promise<VinculoAcesso | null> {
  const { data, error } = await supabase.from("usuarios_perfis").select("*").eq("usuario_id", usuarioId).eq("ativo", true).maybeSingle();
...
export async function obterContextoPersistidoDoUsuario(usuarioId: string, consultar: ConsultarContextoRelacional = consultarContextoRelacional): Promise<ContextoAcesso | null> {
  const { data, error } = await consultar(usuarioId);
  if (error) throw error;
  if (data === null) return null;
  const contexto = mapearContextoRelacional(data, usuarioId);
  return contexto;
}
