...
function exigirContexto(contexto: ContextoOperacionalAtivo | undefined): ContextoOperacionalAtivo {
...
async function executarLista(f: Required<FiltrosNegociacoes>, contexto: ContextoOperacionalAtivo): Promise<ResultadoLista> {
  ...
  q = q.eq("empresa_id", contexto.empresaId).eq("unidade_id", contexto.unidadeId);
...
async function executarObtencao(id: string, contexto: ContextoOperacionalAtivo): Promise<ResultadoUm> { return supabase.from("negociacoes").select("*").eq("id", id).eq("empresa_id", contexto.empresaId).eq("unidade_id", contexto.unidadeId).maybeSingle(); }
...
export async function listarNegociacoesPersistidas(filtros: FiltrosNegociacoes = {}, executar?: ExecutorListagemNegociacoes, contexto?: ContextoOperacionalAtivo): Promise<ListagemNegociacoes> {
  ...
  const { data, error, count } = executar
    ? await executar(completos)
    : await executarLista(completos, exigirContexto(contexto));
...
export async function obterNegociacaoPersistidaPorId(id: string, executar?: ExecutorObtencaoNegociacao, contexto?: ContextoOperacionalAtivo): Promise<Negociacao | null> {   const { data, error } = executar ? await executar(id) : await executarObtencao(id, exigirContexto(contexto));  if (error) throw error;  const negociacao = data ? mapear(data) : null;  return negociacao; }
