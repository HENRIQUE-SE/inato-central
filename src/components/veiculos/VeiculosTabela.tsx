...
export default function VeiculosTabela({ veiculos }: { veiculos: readonly Veiculo[] }) {
  return <div className="overflow-x-auto"><table className="w-full text-left text-sm">
    ...
    <tbody className="divide-y divide-slate-200">{veiculos.map((veiculo) => <tr key={veiculo.id}>
      <td className="px-5 py-4 font-semibold">{veiculo.placa}</td>
      <td className="px-5 py-4">{[veiculo.marca, veiculo.modelo, veiculo.versao].map((parte) => parte?.trim().replace(/\s*\.$/, "").trim()).filter(Boolean).join(" ")}</td>
      ...
      <td className="px-5 py-4"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{ROTULOS_STATUS_VEICULO[veiculo.status]}</span></td>
      <td className="px-5 py-4"><Link href={`/veiculos/${veiculo.id}`} className="font-semibold text-slate-700 hover:text-slate-950">Ver</Link></td>
    </tr>)}</tbody>
  </table></div>;
}
