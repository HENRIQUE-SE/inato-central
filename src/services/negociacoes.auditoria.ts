...
async function registrar(negociacao: Negociacao, veiculo: Veiculo, detalhes: Record<string, string | readonly string[]>, deps: DependenciasAuditoriaNegociacoes): Promise<void> {
  try { const [usuario, contexto] = await Promise.all([deps.obterUsuario(), deps.obterContexto()]); if (!usuario || !contexto) throw new Error(); await deps.persistir(registrarEventoAuditoria({ empresaId: negociacao.empresaId, unidadeId: negociacao.unidadeId, usuarioId: usuario.id, modulo: "negociacoes", acao: detalhes.camposAlterados ? ACOES_AUDITORIA.ALTERAR : negociacao.status === "em_andamento" ? ACOES_AUDITORIA.CRIAR : ACOES_AUDITORIA.ALTERAR, recursoTipo: "negociacao", recursoId: negociacao.id, resultado: RESULTADOS_AUDITORIA.SUCESSO, origem: ORIGENS_AUDITORIA.USUARIO, detalhes: { veiculoId: negociacao.veiculoId, placa: veiculo.placa, interessado: negociacao.interessadoNome, ...detalhes, perfilCodigo: contexto.perfil.codigo, usuarioEmail: usuario.email } })); } catch { throw new Error("Não foi possível registrar a auditoria da negociação."); }
}
