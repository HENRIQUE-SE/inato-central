...
import { registrarAuditoriaAlteracaoVeiculo, registrarAuditoriaConclusaoPreparacaoVeiculo, registrarAuditoriaConclusaoPublicacaoVeiculo, registrarAuditoriaCriacaoVeiculo } from "./veiculos.auditoria";
...
test("registra conclusão da publicação somente com os detalhes permitidos", async () => {
  let persistido: RegistroAuditoria | null = null;
  const anterior = { ...VEICULO, status: STATUS_VEICULO.PRONTO_PARA_ANUNCIAR };
  await registrarAuditoriaConclusaoPublicacaoVeiculo(anterior, { ...anterior, status: STATUS_VEICULO.DISPONIVEL }, {
    obterUsuario: async () => ({ id: "usuario-1", email: "usuario@inato.test" }),
    obterContextoAcesso: async () => CONTEXTO,
    persistir: async (evento) => { persistido = evento; return evento; },
  });
  assert.ok(persistido); const evento: RegistroAuditoria = persistido;
  assert.equal(evento.modulo, "veiculos"); assert.equal(evento.acao, "alterar"); assert.equal(evento.recursoTipo, "veiculo"); assert.equal(evento.recursoId, "veiculo-1");
  assert.deepEqual(evento.detalhes, { placa: "ABC1D23", statusAnterior: "pronto_para_anunciar", statusNovo: "disponivel", perfilCodigo: "administrador", usuarioEmail: "usuario@inato.test" });
  for (const campo of ["renavam", "chassi", "empresaId", "unidadeId"]) assert.equal(campo in (evento.detalhes ?? {}), false);
});
