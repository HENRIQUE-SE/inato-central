...
const oportunidade: Oportunidade = { id: "00000000-0000-4000-8000-000000000201", empresa_id: "empresa", unidade_id: "unidade", proprietario_nome: "Proprietário", telefone: "34999999999", cidade: "Patrocínio", veiculo_informado: "Veículo", placa: "ABC1D23", origem: "teste", status: "novo", created_at: "2026-08-07T00:00:00.000Z" };
...
const dependencias = { obterContextoAutenticado: async () => ({ usuario, contexto: contextoAcesso }), persistir: async (evento: RegistroAuditoria) => { persistidos.push(evento); return evento; } };
...
test("contexto autenticado recebido é reutilizado como organização confiável", async () => {
  ...
  await registrarAuditoriaCriacaoOportunidade(
    oportunidade,
    { ...dependencias, obterContextoAutenticado: async () => { resolucoes += 1; return contextoRecebido; } },
    contextoRecebido
  );
...
test("usuário ausente mantém evento em memória sem persistir ou criar identificador falso", async () => {
  await registrarAuditoriaCriacaoOportunidade(oportunidade, { ...dependencias, obterContextoAutenticado: async () => null });
