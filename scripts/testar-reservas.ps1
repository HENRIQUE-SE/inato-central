...
try{
 ...
 $config=@{compilerOptions=@{target="ES2023";module="commonjs";moduleResolution="node";lib=@("ES2023","DOM");types=@("node");strict=$true;esModuleInterop=$true;skipLibCheck=$true;baseUrl=$projectRoot;paths=@{"@/*"=@("src/*")};rootDir=(Join-Path $projectRoot "src");outDir=$tempRoot};files=@((Join-Path $projectRoot "src\core\reservas\constants.ts"),(Join-Path $projectRoot "src\core\reservas\types.ts"),(Join-Path $projectRoot "src\core\reservas\service.ts"),(Join-Path $projectRoot "src\core\reservas\index.ts"),(Join-Path $projectRoot "src\core\reservas\service.test.ts"),(Join-Path $projectRoot "src\lib\reservas\reservas.repository.ts"),(Join-Path $projectRoot "src\lib\reservas\reservas.repository.test.ts"),(Join-Path $projectRoot "src\services\reservas.service.ts"),(Join-Path $projectRoot "src\services\reservas.service.test.ts"),(Join-Path $projectRoot "src\services\reservas.auditoria.ts"),(Join-Path $projectRoot "src\services\reservas.auditoria.test.ts"))}
 ...
 &node --test (Join-Path $tempRoot "core\reservas\service.test.js") (Join-Path $tempRoot "lib\reservas\reservas.repository.test.js") (Join-Path $tempRoot "services\reservas.service.test.js") (Join-Path $tempRoot "services\reservas.auditoria.test.js");if($LASTEXITCODE-ne 0){throw "Os testes de Reservas falharam."}
